...
{ 
        ...
        player->ViewMatrix = glm::lookAt(player->Position, player->Position + player->Forward, player->Up);
}

void Player_Init(game_context * context) 
{ 
    main_player.Pitch = 0.0f;
    main_player.Yaw = -90.0f;
    main_player.MovementSpeed = 5.0f;
    main_player.LookSpeed = 200.0f;
    main_player.Position = {0.0f, 0.0f, 0.0f};
    main_player.WorldUp = {0.0f, 1.0f, 0.0f};
    main_player.ProjectionMatrix = glm::perspective(glm::radians(45.0f), (float) WINDOW_WIDTH / (float) WINDOW_HEIGHT, 0.1f, 50.0f);
...
{ 
    ...
    if (direction.x * direction.x + direction.y * direction.y + direction.z * direction.z < .1f ) 
    { 
        direction = {0,0,0};
    } else 
    {
...
{ 
    ...
    // just make sure the move dir isn't zero
    if (sqrMag(main_player.MoveDir) > 0.1f) 
    { 
        float velocity = main_player.MovementSpeed * deltaTime;
        main_player.Position += velocity * main_player.MoveDir;
        main_player.ViewMatrix = glm::lookAt(main_player.Position, main_player.Position + main_player.Forward, main_player.Up);
    }
